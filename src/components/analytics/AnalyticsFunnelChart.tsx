...
export function AnalyticsFunnelChart({
  ...
  subtitle = "Visitor progression from initial visit to signed client partnership.",
    ...
    <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm flex flex-col gap-4">
      ...
      {topCount === 0 ? (
        <div className="py-12 text-center text-zinc-400 text-xs font-medium">
          No visitor events recorded in this period yet. Share marketing links to populate funnel metrics.
        </div>
      ) : (
        <div className="space-y-4">
          {funnel.map((stage, index) => {
            const barBg = stageColorClasses[stage.id] || "bg-blue-600";
              ...
              <div key={stage.id} className="space-y-1.5">
                <div className="flex items-baseline justify-between text-xs">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="px-2 py-0.5 rounded-md bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 font-bold text-[10px]">
